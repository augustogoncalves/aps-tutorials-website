다음 API는 'DesignAutomation.js' 파일에서 마지막 줄 'module.exports = router;' 앞에 추가해야 합니다.

- 작업 항목 시작

여기서 실제로 설계 자동화 프로세스가 시작됩니다. 또한 이 엔드포인트는 입력 파일을 OSS 버킷에 업로드하고 출력을 동일한 버킷에 저장하도록 정의합니다. 파일을 식별하는 데 도움이 되도록 입력과 출력 모두 동일한 원래 파일 이름을 사용하지만 접미사(입력 또는 출력)와 타임스탬프가 함께 사용됩니다.
