'routes/' 폴더 안에 'DesignAutomation.js' 파일을 만듭니다.

- 이 파일에서 모든 엔드포인트를 작성하고, 디자인 자동화 SDK 인스턴스 생성, 파일 업로드 및 이 샘플에서 사용되는 몇 가지 유용한 함수와 같은 모든 유틸리티 함수로 구성된 Utils 클래스를 추가합니다.
...
활동을 생성하기 전에 플러그인으로 App Bundle을 정의하고 적절한 엔진을 선택해야 합니다. utils 클래스 뒤에 다음 끝점을 복사하여 붙여넣습니다.
