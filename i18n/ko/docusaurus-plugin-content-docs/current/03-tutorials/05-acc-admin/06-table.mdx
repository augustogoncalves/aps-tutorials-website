...
마지막으로 'Table' 클래스의 나머지 모든 논리를 구현할 준비가 되었습니다.
'Table'클래스는 프로젝트 목록을 포함하여 다양한 유형의 데이터를 처리합니다.
사용자 목록을 작성하고 다음과 같은 기능을 제공합니다.
- 현재 입력 정보를 기반으로 ACC Admin App에서 데이터 목록을 가져옵니다.
- 현재 데이터 목록을 부트스트랩 테이블에 그립니다.
- 현재 데이터 목록을 csv 파일로 내보냅니다.
- csv 파일의 데이터를 ACC 관리자로 가져옵니다.
...
## 테이블 데이터로드
이제 다음 2개의 함수 내에서 데이터를 로드하는 논리를 완료해 보겠습니다.
- initTableTabs (영문)
- 새로고침 테이블

'table.js'를 수정하여 위의 2개 함수 내에서 코드를 업데이트합니다.
...
- Microsoft Excel에서 CSV 파일을 열면 일부 데이터 유형이 설정에 의해 자동으로 변경됩니다.
**startDate** 는 **"2012/03/14"** 유형으로 변경될 수 있으므로 스크린샷과 같이 **"/"** 없이 **"2012-03-14"** 와 같은 다른 유형을 선택해야 합니다. 
그렇지 않으면 CSV 파일로 내보낸 후 날짜가 올바르게 인식되지 않습니다.
