...
이 단계에서는 히스토그램을 표시하는 뷰어 확장을 만듭니다
모든 디자인 요소에서 특정 속성에 대한 값입니다. 우리는 사용할 것입니다
open source library [Chart.js](https://www.chartjs.org)를 사용하여 히스토그램을 표시합니다.
막대 차트 및 원형 차트로.

## 확장 스켈레톤

이전 단계와 마찬가지로 'extensions' 하위 폴더 아래에 새 파일을 만들어 보겠습니다.
이름을 'HistogramExtension.js'로 지정하고 다음 코드로 채웁니다.
...
표준 확장 스캐폴딩 외에도 'findPropertyValueOccurrences'도 정의했습니다.
나중에 원하는 데이터를 수집하는 데 사용할 도우미 메서드(특정
property 값과 이 값을 포함하는 객체)를 사용할 수 있습니다.
...
다음으로, 뷰어에 두 개의 새 버튼을 추가하도록 'HistogramExtension' 클래스를 업데이트해 보겠습니다
도구 모음은 막대 차트를 표시하기 위한 것이고 다른 하나는 원형 차트를 표시하기 위한 것입니다.
...
확장은 어떻게든 Chart.js 라이브러리의 종속성을 가져와야 합니다. 이것 이후 (종속성 얻기
제3자 라이브러리)가 다른 확장에서도 필요할 수 있으며, 'BaseExtension' 클래스를
이를 처리 할 몇 가지 도우미 메소드 :
...
이제 실제 차트 그래픽을 호스팅할 또 다른 사용자 지정 패널을 만들어 보겠습니다. 'HistogramPanel.js' 만들기
파일을 'HistogramExtension.js'가 있는 동일한 폴더에 저장하고 다음 코드를 추가합니다.
...
이제 뷰어에서 차트 확장 프로그램을 테스트할 차례입니다. 새 도구 모음 중 하나를 클릭합니다.
단추. 새 패널은 히스토그램을 보여주는 막대 차트 또는 원형 차트와 함께 표시되어야 합니다
특정 속성에 대해 표시되는 다른 값(드롭다운에서 선택할 수 있음).
막대 또는 파이 세그먼트를 클릭하면 해당 디자인 요소가 모두 분리됩니다
뷰어에서.
