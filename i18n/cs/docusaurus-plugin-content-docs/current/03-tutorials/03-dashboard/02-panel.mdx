...
V tomto kroku vytvoříme další rozšíření prohlížeče, tentokrát představující vlastní prvky uživatelského rozhraní
rovněž. Na lištu prohlížeče přidáme vlastní tlačítko a po kliknutí na tlačítko
Zobrazíme dokovací panel se souhrnem hodnot vlastností v různých prvcích návrhu.

## Kostra rozšíření

Stejně jako u předchozí přípony vytvořte nový soubor v podsložce 'extensions',
Zavolejte ho SummaryExtension.js a naplňte ho následujícím kódem:
...
Nyní vytvoříme nové tlačítko v nástrojové liště prohlížeče. Vzhledem k tomu, že další rozšíření budou potřebovat
tuto funkci také, pojďme aktualizovat naši třídu 'BaseExtension' (v souboru 'BaseExtension.js')
s následujícími pomocnými metodami:
...
Nový kód používá třídy z jmenného prostoru Autodesk.Viewing.UI k vytvoření
novou skupinu tlačítek panelu nástrojů a nové tlačítko panelu nástrojů se specifickým ID, adresou URL ikony,
a zprávu s popiskem.

Teď použijeme tyto pomocné metody ve třídě SummaryExtension:
...
Metoda 'onToolbarCreated' bude automaticky volána prohlížečem, když (a pokud)
Panel nástrojů je připraven. Všimněte si, že prohlížeč může být aktivován v "bezhlavém" režimu,
v takovém případě nebude vytvořeno žádné uživatelské rozhraní a metoda onToolbarCreated nebude volána.

## Vlastní panel

A nakonec aktualizujeme rozšíření 'SummaryExtension' tak, aby když uživatel klikne na
pomocí tlačítka na panelu nástrojů zobrazíme panel vlastního prohlížeče s daty agregovanými z vlastností
aktuálně načteného návrhu.

Začněte vytvořením souboru "SummaryPanel.js" ve stejné složce, kde je "SummaryExtension.js"
...
Nová třída je odvozena z [Autodesk.Viewing.UI.PropertyPanel](https://aps.autodesk.com/en/docs/viewer/v7/reference/UI/PropertyPanel/)
což je standardní uživatelské rozhraní panelu používané pro zobrazení vlastností vybraných návrhových prvků.
V tomto případě použijeme metody 'removeAllProperties' a 'addProperty' k zobrazení různých
druh dat. Vzhledem ke konkrétní sadě ID objektů a názvů vlastností bude SummaryPanel
Zobrazení agregovaných hodnot (počet, součet, průměr, minimum a maximum) těchto vlastností
napříč všemi objekty.

Nakonec přidáme panel do našeho rozšíření:
...
Tentokrát můžeme naše nové rozšíření otestovat přímo v prohlížeči. Zkuste kliknout na nový panel nástrojů
tlačítko pro otevření našeho souhrnného panelu. Na panelu jsou zobrazeny různé agregace vlastností
jsme napevno zakódovali konstantu 'SUMMARY_PROPS' (_Délka_, _Plocha_, _Objem_, _Hustota_,
_Hmotnost_ a _Cena_) pro všechny objekty. A když izolujeme nebo vybereme některé z designů
prvky ručně, agregované hodnoty by to měly také odrážet.
