...
Tento soubor spustí expresní server. V kořenové složce vytvořte soubor 'start.js' s



:::caution

V názvech souborů se u některých nasazení, například Heroku, rozlišují velká a malá písmena. V tomto kurzu použijeme malá písmena.
...
Tento soubor obsluhuje statické soubory (e.g. html) a směruje požadavky rozhraní API. V kořenové složce vytvořte soubor s názvem "server.js" s následujícím obsahem.
...
V kořenové složce vytvořte soubor s názvem "socket.io.js" s následujícím obsahem.
...
V kořenové složce vytvořte soubor s názvem 'config.js' s následujícím obsahem.
...
Zde používáme proměnné prostředí. V době spuštění našeho expresního serveru budou hodnoty těchto proměnných použity pro připojení k APS.

Nyní vytvořte podsložku 'common' ve složce routes a připravte soubor 'routes/common/oauth.js', který bude skutečně požadovat přístupový token od APS. To se znovu použije v jiných částech tohoto kurzu.
...
Projekt je připraven! V tomto okamžiku by váš projekt měl vypadat takto
