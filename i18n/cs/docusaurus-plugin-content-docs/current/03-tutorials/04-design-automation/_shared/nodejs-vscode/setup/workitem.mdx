Následující rozhraní API by měla být přidána do souboru 'DesignAutomation.js' před poslední řádek 'module.exports = router;'

- StartWorkitem

To je místo, kde vlastně začínáme proces automatizace návrhu. Tento koncový bod také nahraje vstupní soubor do kontejneru OSS a definuje, že výstup by měl být uložen ve stejném kontejneru. Aby bylo možné soubory snáze identifikovat, používá vstup i výstup stejný původní název souboru, ale s příponou (vstup nebo výstup) a časovým razítkem.
