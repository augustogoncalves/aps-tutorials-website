Následující rozhraní API by měla být přidána do souboru DesignAutomationController.cs před poslední řádek metody GetDefinedActivities.

- StartWorkitem

To je místo, kde vlastně začínáme s automatizací návrhu. Tento koncový bod také nahraje vstupní soubor do kontejneru OSS a definuje, že výstup by měl být uložen ve stejném kontejneru. Aby bylo možné soubory snáze identifikovat, používá vstup i výstup stejný původní název souboru, ale s příponou (vstup nebo výstup) a časovým razítkem.
...
V případě reálné aplikace byste se spoléhali na [mechanismus zpětného volání](https://aps.autodesk.com/en/docs/design-automation/v3/developers_guide/callbacks/) automatizace návrhu místo použití dotazování, abyste věděli, kdy se pracovní položka dokončila
...
V neposlední řadě, abychom vám pomohli s testováním, toto rozhraní API odstraní z vašeho účtu všechny balíčky aplikací a aktivity.
