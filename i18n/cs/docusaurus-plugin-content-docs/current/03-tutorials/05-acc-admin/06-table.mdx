...
Nakonec jsme připraveni implementovat veškerou zbývající logiku třídy 'Table'.
Třída 'Table' je zodpovědná za zpracování různých typů dat, včetně seznamu projektů,
seznam uživatelů a poskytovat funkce včetně:
- Získejte seznam dat z aplikace ACC Admin App na základě aktuálních vstupních informací.
- Nakreslete aktuální seznam dat do zaváděcí tabulky.
- Exportujte aktuální seznam dat do souboru csv.
- Importujte data ze souboru CSV do správce ACC.
...
## Načíst data tabulky
Teď dokončíme logiku pro načtení dat v rámci následujících 2 funkcí:
- initTableTabs
- refreshTable (Tabulka pro osvěžení)

Aktualizujte kód v rámci výše uvedených 2 funkcí úpravou 'table.js':
...
- Pokud otevřete CSV soubor pomocí Microsoft Excel, nastavení se automaticky změní některý datový typ, například sloupec
**startDate** může být změněn na typ jako **"2012/03/14"**, ujistěte se, že jste vybrali jiný typ, jako je **"2012-03-14"** bez **"/"**, jak je znázorněno na snímku obrazovky, 
jinak nebude datum po exportu do souboru CSV správně rozpoznáno.
