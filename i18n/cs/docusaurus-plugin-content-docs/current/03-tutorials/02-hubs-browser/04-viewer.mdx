...
- 'initViewer' vytvoří novou instanci prohlížeče v zadaném kontejneru DOM
- 'loadModel' pro načtení konkrétního modelu do prohlížeče

## Logika postranního panelu

Dále implementujeme chování postranního panelu, kde se budeme zobrazovat
Všechny rozbočovače, projekty, složky, položky a verze v komponentě stromového zobrazení třetí strany.
...
Pokud ano, kód pak může inicializovat prohlížeč i komponentu stromového zobrazení.
Funkce zpětného volání předaná 'initTree' zajišťuje, že když klikneme na uzel typu list
Ve stromu začne prohlížeč načítat příslušný model.
...
A to je vše! Vaše aplikace je nyní připravena k akci. Spusťte jej jako obvykle, a když přejdete na
[http://localhost:8080](http://localhost:8080), mělo by se vám zobrazit jednoduché uživatelské rozhraní s
Stromové zobrazení na levé straně a prázdný prohlížeč na pravé straně. Zkus listovat stromem,
a vyberte konkrétní verzi jednoho ze svých souborů. Poté by měl být načten odpovídající model
do diváka.
...
Vezměte prosím na vědomí, že návrhy nahrané do Fusion Teams nejsou zpracovány pro automatické prohlížení.
Tyto návrhy budete chtít nejprve otevřít na webu Fusion Teams, a až budou připraveny,
můžete si je prohlédnout ve své vlastní aplikaci Hubs Browser.
