Vytvořte soubor "APS.cs" v podsložce "Modely". To je místo, kde budeme realizovat
logiku specifickou pro APS, která bude použita v různých oblastech naší serverové aplikace. Pojďme
...
Dále vytvoříme 'APS. Auth.cs" v podsložce Models s následujícím kódem:
...
Tyto pomocné metody budou později použity v řadičích našeho serveru pro zpracování
různé typy požadavků souvisejících s ověřováním, například přesměrování uživatele
na přihlašovací stránku společnosti Autodesk, kde se zpracuje zpětné volání, když je uživatel přesměrován zpět
do naší aplikace nebo aktualizace tokenů, jejichž platnost vypršela.

Nakonec aktualizujeme náš soubor 'Startup.cs' tak, aby vytvořila singleton instanci třídy 'APS'
