...
Enfin, nous sommes prêts à implémenter toute la logique restante de la classe 'Table'.
La classe 'Table' est chargée de traiter différents types de données, y compris la liste des projets,
et fournir les fonctions suivantes :
- Obtenez la liste des données de l’application ACC Admin en fonction des informations d’entrée actuelles.
- Dessinez la liste de données actuelle dans une table d’amorçage.
- Exportez la liste de données actuelle dans un fichier csv.
- Importez les données d’un fichier csv dans l’administrateur ACC.
...
Maintenant, complétons la logique pour charger les données dans les 2 fonctions suivantes :
- initTableTabs (en anglais seulement)
- table de rafraîchissement

Mettez à jour le code dans les 2 fonctions ci-dessus en modifiant 'table.js' :
...
- Si vous ouvrez le fichier CSV avec Microsoft Excel, certains types de données seront automatiquement modifiés par le paramètre, par exemple, la colonne de
**startDate** peut être changé en **"2012/03/14"**, assurez-vous de sélectionner un autre type comme **"2012-03-14"** sans **"/"** comme indiqué dans la capture d’écran, 
sinon, la date ne sera pas reconnue correctement après l’exportation vers un fichier CSV.
