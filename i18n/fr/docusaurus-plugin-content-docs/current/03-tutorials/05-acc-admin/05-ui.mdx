...
Ok, nous allons maintenant construire la partie côté client de notre application.

## Espace réservé au tableau

Commençons par implémenter une fonction d’espace réservé de Table,
Créez un fichier 'table.js' sous le sous-dossier 'wwwroot' avec le code suivant :
...
qui définissent une classe 'Table' qui est utilisée pour lister les informations des Projets et des Utilisateurs, le module aussi
exporte deux fonctions :

- 'initTableTabs' créera les onglets et enregistrera également la fonction lorsque l’onglet actif est modifié.
- 'refreshTable' pour charger les données du hub/projet sélectionné dans la table.

## Logique de la barre latérale

Ensuite, nous allons implémenter le comportement d’une barre latérale où nous allons afficher
tous les hubs et projets ACC dans un composant d’arborescence tiers.
...
Si c’est le cas, le code peut alors initialiser la table ainsi que le composant d’arborescence.
La fonction de rappel passée à 'initTree' s’assure que lorsque nous cliquons sur un nœud terminal
Dans l’arborescence, la table commencera à charger les données correspondantes.
...
Votre application est maintenant prête à être testée pour le framework. Démarrez-le comme d’habitude, et lorsque vous allez à
[http://localhost:8080](http://localhost:8080), vous devriez être présenté avec une interface utilisateur simple, avec
une arborescence sur le côté gauche, et une table vide sur la droite. Essayez de parcourir l’arbre,
Nous chargerons les données dans le tableau à l’étape suivante.
