Les méthodes suivantes doivent être ajoutées à la classe 'DesignAutomationController'.

- Attributs du moteur

Pour définir l’activité, nous aurons besoin de l’exécutable et de l’extension de fichier par défaut. Cette fonction d’assistance le fournit (à partir du nom du moteur).
...
Définissez une nouvelle activité avec un fichier d’entrée, des données d’entrée (JSON) et un fichier de sortie.
...
- GetDefinedActivities (en anglais seulement)

Nous aurons également besoin d’une méthode pour renvoyer toutes les activités définies. Notez que ne renvoie que ceux que vous avez définis (nous utilisons l’ID du client APS comme pseudo, qui apparaît ensuite comme préfixe).
