Les API suivantes doivent être ajoutées au fichier 'DesignAutomationController.cs' avant la dernière ligne de la méthode 'GetDefinedActivities'.

- StartWorkitem

C’est là que nous commençons l’automatisation de la conception. Ce point de terminaison charge également le fichier d’entrée dans un compartiment OSS et définit que la sortie doit être enregistrée dans le même compartiment. Pour vous aider à identifier les fichiers, l’entrée et la sortie utilisent le même nom de fichier d’origine, mais avec un suffixe (entrée ou sortie) plus un horodatage.
...
- Élément de travail MonitorWorkitem
...
Dans le cas d’une application réelle, vous vous appuieriez sur le [mécanisme de rappel](https://aps.autodesk.com/en/docs/design-automation/v3/developers_guide/callbacks/) de Design Automation au lieu d’utiliser l’interrogation pour savoir quand l’élément de travail s’est terminé
...
- ClearAccount (en anglais seulement)

Enfin, pour vous aider à tester, cette API supprime tous les appbundles et activités de votre compte.
