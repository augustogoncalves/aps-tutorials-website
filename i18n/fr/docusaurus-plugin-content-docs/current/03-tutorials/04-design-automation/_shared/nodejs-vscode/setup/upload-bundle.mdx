Dans le dossier 'routes/', créez le fichier 'DesignAutomation.js'.

- Dans ce fichier, nous allons écrire tous les points de terminaison, nous allons ajouter la classe Utils composée de toutes les fonctions utilitaires telles que la création d’une instance SDK d’automatisation de la conception, le téléchargement de fichiers et quelques autres fonctions utiles qui sont utilisées dans cet exemple.
...
Avant de créer une activité, nous devons définir le bundle d’applications avec le plugin et sélectionner le moteur approprié. Copiez et collez les points de terminaison suivants après la classe utils.
