...
このステップでは、別のビューア拡張機能を作成し、今回はカスタム UI 要素を導入します
にも。ビューアのツールバーにカスタムボタンを追加し、ボタンがクリックされると、
さまざまな設計要素のプロパティ値の概要を含むドッキング パネルを表示します。

## 拡張機能のスケルトン

前の拡張子と同様に、「extensions」サブフォルダーの下に新しいファイルを作成します。
これを 'SummaryExtension.js' と呼び、次のコードを設定します。
...
次に、ビューアツールバーに新しいボタンを作成します。他の拡張機能は
この機能も、(「BaseExtension.js」ファイル内の)「BaseExtension」クラスを更新しましょう
を次のヘルパーメソッドで置き換えます。
...
新しいコードでは、'Autodesk.Viewing.UI' 名前空間のクラスを使用して、
新しいツールバーボタングループ、および特定のID、アイコンURL、
とツールチップメッセージ。

次に、これらのヘルパーメソッドを 'SummaryExtension' クラスで使用しましょう。
...
'onToolbarCreated'メソッドは、次の場合(およびif)にビューアによって自動的に呼び出されます
ツールバーの準備が整いました。ビューアは「ヘッドレス」モードで起動される可能性があることに注意してください。
この場合、UI は作成されず、'onToolbarCreated' メソッドは呼び出されません。

## カスタムパネル

そして最後に、「SummaryExtension」拡張機能を更新して、ユーザーが
ツールバーボタンをクリックすると、プロパティから集計されたデータを含むカスタムビューアパネルが表示されます
現在読み込まれているデザインの。

まず、「SummaryExtension.js」と同じフォルダに「SummaryPanel.js」ファイルを作成します。
が見つかったら、次のコードを追加します。
...
新しいクラスは [Autodesk.Viewing.UI.PropertyPanel](https://aps.autodesk.com/en/docs/viewer/v7/reference/UI/PropertyPanel/) から派生します
これは、選択した設計要素のプロパティを表示するために使用される標準パネル UI です。
この場合、'removeAllProperties' メソッドと 'addProperty' メソッドを使用して、異なる
データの種類。オブジェクト ID とプロパティ名の特定のセットを指定すると、'SummaryPanel' は
これらのプロパティの集計値 (カウント、合計、平均、最小、最大) を表示します
すべてのオブジェクトに渡って。

最後に、拡張機能にパネルを追加しましょう。
...
今回は、新しい拡張機能をビューアで直接テストできます。新しいツールバーをクリックしてみてください
ボタンをクリックして、概要パネルを開きます。パネルには、プロパティのさまざまな集計が表示されます
'SUMMARY_PROPS' 定数 (_Length_, _Area_, _Volume_, _Density_,
_Mass_、および _Price_) をすべてのオブジェクトに適用します。そして、デザインの一部を分離または選択すると、
要素を手動で使用する場合、集計された値にもそれを反映する必要があります。
