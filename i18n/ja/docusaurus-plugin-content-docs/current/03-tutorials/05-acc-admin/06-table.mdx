...
最後に、クラス「Table」の残りのロジックをすべて実装する準備が整いました。
クラス 'Table'は、プロジェクトリスト、
ユーザーリストを作成し、次のような機能を提供します。
- 現在の入力情報に基づいて、ACC管理アプリからデータリストを取得します。
- 現在のデータリストをブートストラップテーブルに描画します。
- 現在のデータリストをcsvファイルにエクスポートします。
- csv ファイルから ACC 管理画面にデータをインポートします。
...
## テーブルデータの読み込み
それでは、次の2つの関数内でデータをロードするロジックを完成させましょう。
- initTableTabs (英語)
- リフレッシュテーブル

'table.js' を変更して、上記の 2 つの関数内のコードを更新します。
...
- CSVファイルをMicrosoft Excelで開くと、一部のデータ型は、
**startDate** は **"2012/03/14"** のようなタイプに変更される可能性がありますが、スクリーンショットに示すように、**"/"** なしで **"2012-03-14"** のような別のタイプを選択してください。
そうしないと、CSVファイルにエクスポートした後、日付が正しく認識されません。
