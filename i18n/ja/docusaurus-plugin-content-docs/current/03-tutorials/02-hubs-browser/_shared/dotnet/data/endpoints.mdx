「Controllers」サブフォルダの下に、次の内容の「HubsController.cs」ファイルを作成します。
...
コントローラは、他のAPSベースのコンテンツを参照するために複数のエンドポイントを処理します
BIM 360 Docs や ACC などのアプリケーション。これらのエンドポイントは、次の場合に利用します
アプリケーションの UI 部分をビルドする。