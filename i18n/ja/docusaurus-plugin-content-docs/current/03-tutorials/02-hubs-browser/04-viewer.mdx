...
これは、次の 2 つの関数をエクスポートします。

- 'initViewer' は、指定された DOM コンテナにビューアの新しいインスタンスを作成します
- 'loadModel' は、特定のモデルをビューアにロードします。

## サイドバーのロジック

次に、表示するサイドバーの振る舞いを実装します
すべてのハブ、プロジェクト、フォルダー、アイテム、およびバージョンを 3rd パーティのツリービューコンポーネントに含めます。
...
もしそうなら、コードはビューアとツリービューコンポーネントを初期化できます。
'initTree'に渡されるコールバック関数は、リーフノードをクリックしたときに
ツリーで、ビューアは対応するモデルの読み込みを開始します。
...
以上です!これで、アプリケーションを動作させる準備が整いました。通常どおりに起動し、
[http://localhost:8080](http://localhost:8080) の場合、シンプルな UI が表示されます。
左側にツリービュー、右側に空のビューアがあります。ツリーをブラウズしてみて、
をクリックし、ファイルの 1 つの特定のバージョンを選択します。その後、対応するモデルをロードする必要があります
をビューアに挿入します。
...
Fusion Teamsにアップロードされたデザインは、自動的には表示用に処理されないことに注意してください。
これらのデザインは、まず Fusion Teams の Web サイトで開き、準備ができたら、
これらは、独自のHubsブラウザアプリケーションで表示できます。
