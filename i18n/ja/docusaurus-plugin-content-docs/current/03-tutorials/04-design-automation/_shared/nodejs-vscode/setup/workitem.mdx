...
ここから、実際に設計自動化プロセスを開始します。また、このエンドポイントは、入力ファイルを OSS バケットにアップロードし、出力を同じバケットに保存するように定義します。ファイルを識別しやすくするために、入力と出力の両方で同じ元のファイル名が使用されますが、接尾辞 (入力または出力) とタイム スタンプが付きます。
