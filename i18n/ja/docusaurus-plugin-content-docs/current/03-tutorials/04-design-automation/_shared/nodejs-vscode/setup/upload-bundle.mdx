「routes/」フォルダ内に「DesignAutomation.js」ファイルを作成します。

- このファイルでは、すべてのエンドポイントを記述し、設計自動化SDKインスタンスの作成、ファイルのアップロード、およびこのサンプルで使用されるいくつかの便利な関数など、すべてのユーティリティ関数で構成されるUtilsクラスを追加します。
...
アクティビティを作成する前に、プラグインを使用して App Bundle を定義し、適切なエンジンを選択する必要があります。次のエンドポイントを utils クラスの後にコピー&&ペーストします。
