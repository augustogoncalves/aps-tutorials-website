Los siguientes métodos deben agregarse a la clase 'DesignAutomationController'.

- Atributos del motor

Para definir la actividad necesitaremos el ejecutable y la extensión de archivo por defecto. Esta función auxiliar lo proporciona (desde el nombre del motor).
...
- CreateActivity (Actividad de creación)

Defina una nueva actividad con un archivo de entrada, datos de entrada (JSON) y un archivo de salida.
...
- GetDefinedActivities (Actividades Definidas)

También necesitaremos un método para devolver todas las actividades definidas. Tenga en cuenta que devuelve solo los definidos por usted (usamos el ID de cliente APS como nombre de apodo, que luego aparece como prefijo).
