...
Bien, ahora construiremos la parte del lado del cliente de nuestra aplicación.

## Marcador de posición de mesa

Comencemos implementando las funciones de marcador de posición de Table,
Cree un archivo 'table.js' en la subcarpeta 'wwwroot' con el siguiente código:
...
que definen una clase 'Tabla' que se utiliza para listar la información de Proyectos y Usuarios, el módulo también
Exporta dos funciones:

- 'initTableTabs' creará las pestañas y también registrará la función cuando se cambie la pestaña activa.
- 'refreshTable' para cargar los datos del hub/proyecto seleccionado en la tabla.

## Lógica de la barra lateral

A continuación, implementaremos el comportamiento de una barra lateral donde vamos a mostrar
todos los centros y proyectos de ACC en un componente de vista de árbol de terceros.
...
Si es así, el código puede inicializar la tabla, así como el componente de vista de árbol.
La función de devolución de llamada pasada a 'initTree' se asegura de que cuando hacemos clic en un nodo hoja
En el árbol, la tabla comenzará a cargar los datos correspondientes.
...
La aplicación está lista para probar el marco. Comience como de costumbre, y cuando vaya a
[http://localhost:8080](http://localhost:8080), se le debe presentar una interfaz de usuario simple, con
una vista de árbol en el lado izquierdo y una mesa vacía a la derecha. Intenta navegar a través del árbol,
Cargaremos los datos en la tabla en el siguiente paso.
