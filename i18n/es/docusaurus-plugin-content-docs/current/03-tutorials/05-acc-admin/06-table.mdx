...
Finalmente, estamos listos para implementar toda la lógica restante de la clase 'Table'.
La clase 'Table' es responsable de manejar diferentes tipos de datos, incluida la lista de proyectos,
lista de usuarios, y proporcionar las funciones que incluyen:
- Obtenga la lista de datos de la aplicación ACC Admin en función de la información de entrada actual.
- Dibuje la lista de datos actual en una tabla de arranque.
- Exporte la lista de datos actual a un archivo csv.
- Importe los datos de un archivo csv al administrador de ACC.
...
Ahora, completemos la lógica para cargar los datos dentro de las siguientes 2 funciones:
- initTableTabs
- refreshTable (en inglés)

Actualice el código dentro de las 2 funciones anteriores modificando 'table.js':
...
- Si abre el archivo CSV con Microsoft Excel, algún tipo de datos cambiará automáticamente mediante la configuración, por ejemplo, la columna de
**startDate** puede cambiarse al tipo **"2012/03/14"**, asegúrese de seleccionar otro tipo como **"2012-03-14"** sin **"/"** como se muestra en la captura de pantalla, 
de lo contrario, la fecha no se reconocerá correctamente después de exportar a un archivo CSV.
