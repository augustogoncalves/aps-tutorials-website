...
Ok, vamos construir a parte do lado do cliente do nosso aplicativo agora.

## Espaço reservado para mesa

Vamos começar implementando uma função de espaço reservado de Tabela,
Crie um arquivo 'table.js' na subpasta 'wwwroot' com o seguinte código:
...
que definem uma classe 'Tabela' que é usada para listar as informações de Projetos e Usuários, o módulo também
exporta duas funções:

- 'initTableTabs' criará as Guias e também registrará a função quando a guia ativa for alterada.
- 'refreshTable' para carregar os dados do hub/projeto selecionado na tabela.

## Lógica da barra lateral

Em seguida, implementaremos o comportamento de uma barra lateral onde exibiremos
todos os hubs e projetos ACC em um componente de exibição em árvore de terceiros 3rd.
...
Se estivermos, o código pode então inicializar a tabela, bem como o componente de exibição em árvore.
A função de retorno de chamada passada para 'initTree' garante que quando clicamos em um nó de folha
Na árvore, a tabela começará a carregar os dados correspondentes.
...
Seu aplicativo agora está pronto para testar a estrutura. Inicie como de costume, e quando você for para
[http://localhost:8080](http://localhost:8080), você deve ser apresentado com uma interface do usuário simples, com
uma vista em árvore no lado esquerdo e uma tabela vazia à direita. Tente navegar pela árvore,
Carregaremos dados na tabela na próxima etapa.
