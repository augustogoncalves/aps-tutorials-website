...
Finalmente, estamos prontos para implementar toda a lógica restante da classe 'Table'.
A classe 'Table' é responsável por lidar com diferentes tipos de dados, incluindo lista de projetos,
lista de usuários e fornecer as funções, incluindo:
- Obtenha a lista de dados do ACC Admin App com base nas informações de entrada atuais.
- Desenhe a lista de dados atual em uma tabela de bootstrap.
- Exportar a lista de dados atual para um arquivo csv.
- Importe os dados de um arquivo csv para o ACC Admin.
...
Agora, vamos concluir a lógica para carregar os dados dentro das seguintes 2 funções:
- initTableTabs
- refreshTable

Atualize o código dentro das 2 funções acima modificando 'table.js':
...
- Se você abrir o arquivo CSV com o Microsoft Excel, algum tipo de dados será alterado automaticamente pela configuração, por exemplo, a coluna de
**startDate** pode ser alterado para o tipo como **"2012/03/14"**, certifique-se de selecionar outro tipo como **"2012-03-14"** sem **"/"** como mostrado na captura de tela, 
caso contrário, a data não será reconhecida corretamente após a exportação para o arquivo CSV.
