Dentro da pasta 'routes/' crie o arquivo 'DesignAutomation.js'.

- Neste arquivo vamos escrever todos os endpoints, vamos adicionar a classe Utils que consiste em todas as funções utilitárias como a criação de instância SDK de automação de projeto, upload de arquivo e algumas funções mais úteis que são usadas neste exemplo.
...
Antes de criar a atividade, precisamos definir o pacote de aplicativos com o plugin e selecionar o mecanismo apropriado. Copie & cole os seguintes pontos de extremidade após a classe utils.
