Os seguintes métodos devem ser adicionados à classe 'DesignAutomationController'.

- EngineAttributes

Para definir a atividade, precisaremos do executável e da extensão de arquivo padrão. Esta função auxiliar fornece-lo (a partir do nome do mecanismo).
...
Defina uma nova atividade com um arquivo de entrada, dados de entrada (JSON) e um arquivo de saída.
...
Também precisaremos de um método para retornar todas as atividades definidas. Observe que retorna apenas aqueles definidos por você (usamos o ID do Cliente APS como apelido, que aparece como um prefixo).
