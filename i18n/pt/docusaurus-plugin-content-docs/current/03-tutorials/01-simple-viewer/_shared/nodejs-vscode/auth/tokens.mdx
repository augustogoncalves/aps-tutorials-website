...
O código fornece duas funções auxiliares - uma para gerar tokens de acesso para uso interno
(dando-nos acesso de leitura/gravação aos buckets e objetos de Gerenciamento de Dados) e um para gerar
tokens para uso público (dando apenas um acesso de leitura às saídas de tradução da Derivada de Modelo
serviço).