...
最后，我们准备实现类 'Table' 的所有剩余逻辑。
类“Table”负责处理不同类型的数据，包括项目列表、
用户列表，并提供以下功能：
- 根据当前输入信息从ACC管理应用程序获取数据列表。
- 将当前数据列表绘制到引导表中。
- 将当前数据列表导出到 csv 文件中。
- 将数据从 csv 文件导入 ACC Admin。
...
## 加载表数据
现在，让我们完成在以下 2 个函数中加载数据的逻辑：
- 初始化表选项卡
- 刷新表

通过修改“table.js”来更新上述 2 个函数中的代码：
...
- 如果您使用 Microsoft Excel 打开 CSV 文件，则某些数据类型将通过设置自动更改，例如，列
**startDate** 可能会更改为 **“2012/03/14”** 类型，请确保选择其他类型，如 **“2012-03-14”** 不带 **“/”** 如屏幕截图所示，
否则，导出到CSV文件后将无法正确识别日期。
