在“routes/”文件夹中创建“DesignAutomation.js”文件。

- 在此文件中，我们将编写所有端点，我们将添加 Utils 类，该类由所有实用程序函数组成，例如创建设计自动化 SDK 实例、上传文件以及此示例中使用的一些更有用的函数。
...
在创建活动之前，我们需要使用插件定义 app bundle 并选择合适的引擎。将以下端点复制并粘贴到 utils 类之后。
