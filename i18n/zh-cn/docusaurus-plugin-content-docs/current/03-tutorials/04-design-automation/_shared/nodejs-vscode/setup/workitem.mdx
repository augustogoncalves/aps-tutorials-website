应将以下 API 添加到“DesignAutomation.js”文件中，在最后一行“module.exports = router;

- 启动工作项

这就是我们真正开始设计自动化过程的地方。该终端节点还会将输入文件上传到 OSS Bucket，并定义输出应保存在同一 Bucket 中。为了帮助您识别文件，输入和输出都使用相同的原始文件名，但带有后缀（输入或输出）和时间戳。
