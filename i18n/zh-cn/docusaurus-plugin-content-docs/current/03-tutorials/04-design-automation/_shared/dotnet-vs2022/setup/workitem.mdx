应将以下 API 添加到“DesignAutomationController.cs”文件的最后一行“GetDefinedActivities”方法之前。

- 启动工作项

这就是我们真正开始设计自动化的地方。该终端节点还会将输入文件上传到 OSS Bucket，并定义输出应保存在同一 Bucket 中。为了帮助您识别文件，输入和输出都使用相同的原始文件名，但带有后缀（输入或输出）和时间戳。
...
对于实际应用程序，您将依赖于设计自动化的 [回调机制](https://aps.autodesk.com/en/docs/design-automation/v3/developers_guide/callbacks/），而不是使用轮询来了解工作项何时完成
...
最后但并非最不重要的一点是，为了帮助您进行测试，此 API 会从您的帐户中删除所有 appbundle 和活动。
